...
class ProductsImport
  ...
  def perform
    import_log = ImportLog.create(log_type: 'products_import', state: 'Iniciando')
  ...
  private 
    def load_product_to_db(xml_string, import_log)
      ...
      CSV.parse(products_table, {:col_sep=>"\t", :headers=>true}) do |row|
        ...
        product_to_find = product_present?(row['ProductCode'], row['ProductName'], row['Options'], row['SizeCode'], row['FlavorCode'], row['OptionSelectionGroupType'], row['ProductOptionSelectionGroup'])
        if  product_to_find.nil? || (product_to_find.discontinued.present? && product_to_find.discontinued == true)
          ...
          import_log.import_events.create(name: 'Nuevo producto agregaro', subject: row['ProductCode'] , message: "Se agrego #{row['ProductName']}")
        else
          import_log.import_events.create(name: 'Conflicto de Importación', subject:product_to_find.id , message: 'Durante la importación se encontro un product identico, pero esta no descontinuado')
        end
    ...
    def product_present?(productcode, producname, options, sizecode, flavorcode, optionselectiongrouptype, productoptionselectiongroup)
      Product.where(productcode: productcode, productname: producname, options: options,
       sizecode: sizecode, flavorcode: flavorcode, optionselectiongrouptype: optionselectiongrouptype, productoptionselectiongroup: productoptionselectiongroup).first
    end
end
